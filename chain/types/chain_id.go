...
var (
	...
	regexSeparator = `-{1}`
	regexEpoch     = `[1-9][0-9]*`
	fluxChainID    = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)$`, regexChainID, regexSeparator, regexEpoch))
)
...
func IsValidChainID(chainID string) bool {
	...
	return fluxChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := fluxChainID.FindStringSubmatch(chainID)
...
func GenerateRandomChainID() string {
	return fmt.Sprintf("flux-%d", 10+tmrand.Intn(10000))
}
